    ...
    if (decision.isDenied()) {
      if (decision.reason.isSensitiveInfo()) {
        throw errors.BAD_REQUEST({
          message:
            "Sensitive information detected. Please remove PII (e.g. creadit cards details / phone numbers).",
        });
      }
      ...
      if (decision.reason.isRateLimit()) {
        throw errors.RATE_LIMITED({
          message:
            "Too many requests. Please wait at least 1 min and then try again.",
        });
      }

      if (decision.reason.isBot()) {
        throw errors.FORBIDDEN({ message: "Automated trafic blocked." });
      }
